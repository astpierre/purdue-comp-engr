	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_MALLOC:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
      ihandle = (int)malloc(ihandle,currentPCB);
      ProcessSetResult(currentPCB, ihandle); //Return handle
      break;
    case TRAP_MFREE:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
      ihandle = mfree((void*)ihandle,currentPCB);
      ProcessSetResult(currentPCB, ihandle); //Return handle
      break;
    ...
    case TRAP_ADDRESS:
      ...
      //exitsim ();
      printf(" (traps.c): TRAP_ADDRESS, illegal address at iar=0x%x, isr=0x%x\n", iar, isr);
      printf(" (traps.c): Rather than call exitsim(), calling ProcessKill()\n");
      ProcessKill();
      break;
